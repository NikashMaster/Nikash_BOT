...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({ pattern: 'rwolf ?(.*)', fromMe: true,dontAddCommandList: false }, (async (message, match) => {
        ...
        var ttinullimage = await axios.get(`https://api.zeks.xyz/api/wolflogo?apikey=cjxJIn9eDletRHac3CG6CdvGxFH&text1=${encodeURIComponent(match[1])}&text2=RAVAN`, { responseType: 'arraybuffer' })
    ...
    Asena.addCommand({ pattern: 'rwolf ?(.*)', fromMe: false,dontAddCommandList: false }, (async (message, match) => {
        ...
        var ttinullimage = await axios.get(`https://api.zeks.xyz/api/wolflogo?apikey=cjxJIn9eDletRHac3CG6CdvGxFH&text1=${encodeURIComponent(match[1])}&text2=RAVANA`, { responseType: 'arraybuffer' })
